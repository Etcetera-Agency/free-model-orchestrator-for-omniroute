...
def reconcile_roles(
    roles: dict[str, dict], *, desired_roles: set[str], now: datetime, grace_period: timedelta
) -> dict[str, dict]:
    ...
    for role_id, role in list(result.items()):
        if role_id in desired_roles:
            ...
            continue
        if (
            role.get("status") == "retiring"
            and role.get("missing_since")
            and now - role["missing_since"] > grace_period
            and role.get("recent_usage", 0) == 0
        ):
