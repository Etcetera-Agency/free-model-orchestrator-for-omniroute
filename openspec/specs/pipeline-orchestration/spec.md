...
free/0-cost status changed. The trigger SHALL be restricted to models reachable
via an existing OmniRoute connection; otherwise the stage is skipped as an
idempotent no-change. On a triggered run the pipeline SHALL continue through
scoring, allocation, diff, and apply so that a model that gained free status is
added as a member of any existing combo whose band and capability it fits
without creating a combo, and a model that lost free status is dropped from
combos on rebalance and its quota rule is deactivated.
