...
## Purpose
Keep Hermes profile model slots pointed at existing OmniRoute combos without
letting the orchestrator create operator-owned combos.
...
rewritten to the existing combo whose members include the slot's canonical model
(provider stripped). When no existing combo contains that canonical model, the
slot SHALL be rewritten to the default combo: the main combo of the `default`
profile. A slot already pointing at an existing `fmo-` combo, or set to
`auto`/empty, SHALL be left unchanged. The operation SHALL NOT create or delete
OmniRoute combos.
