...
- **THEN** the runtime first refreshes the current OmniRoute provider/model
  catalog and only tests active models from that refreshed catalog through
  OmniRoute's `/api/models/test` model-test route with the configured limit,
