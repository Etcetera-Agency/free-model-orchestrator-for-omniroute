...
discarded in-memory result. Artificial Analysis model metrics SHALL be persisted
both as provider-quality observations and as canonical
`artificial_analysis_model_metrics` rows used by the scorer. A dry-run SHALL
fetch and validate but persist nothing.
...
- **THEN** the models.dev candidates and the AA snapshot are persisted via the
  repository layer
- **AND** AA metrics are available to role scoring through canonical model metric
  rows
