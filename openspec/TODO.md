...
1. `add-auxiliary-slot-consumers` — emit a consumer per non-`auto` auxiliary slot
   (and gateway/platform aux); demand sums per shared combo. Depends on archived
   `update-hermes-source-to-per-profile-config`.
2. `add-forecast-driven-quality-band` — add `roles.maximum_quality_value`; write
   ...
   order inverted to ascending (smartest last). Depends on (1) and archived
   `update-combo-applier-to-rebalance-only`.
3. `add-profile-combo-normalization` — `normalize-profiles` CLI: rewrite a raw or
   ...
   `update-hermes-source-to-per-profile-config`,
   `update-combo-applier-to-rebalance-only`, and (2).
4. `trigger-quota-recalc-on-free-model-changes` — gate quota-research on a
   ...
   archived `update-combo-applier-to-rebalance-only` and (2).
5. `register-new-free-models-in-omniroute` — register a new confirmed-free model
   ...
   `trigger-quota-recalc-on-free-model-changes` (4) and archived
   `update-combo-applier-to-rebalance-only`.
