...
- [ ] 1.1 Write failing test: `_demand_forecast_stage`, `_allocation_stage`,
      ...
      dedicated modules`.
- [ ] 1.2 Write failing test: `fmo/composition_stages/_legacy.py` does not exist
...
- [ ] 2.1 Move `_diff_stage`, `_apply_stage`, the apply-safety checks, the review
...
- [ ] 2.2 Move `_demand_forecast_stage`, `_allocation_stage`,
      `_configured_router_input`, `_capacity_weight` into `allocation.py`.
- [ ] 2.3 Move `_rollback_stage`, `_rollback_targets`, `_rollback_combo_id` into
      `rollback.py`.
- [ ] 2.4 Move `_audit_stage` into `audit.py`.

## 3. Relocate the shared spine

- [ ] 3.1 Move the cross-cluster helper bodies (`_effect_result`,
...
- [ ] 3.2 Move the `Stage*` / `FreeModelChanges` dataclasses and the
      `StageAdapter` type alias into a new `_base.py`.
- [ ] 3.3 Move the base `_production_stage_adapters()` builder into `_base.py` and
...
- [ ] 4.1 Delete `src/fmo/composition_stages/_legacy.py`; drop every
      `from . import _legacy` / `from ._legacy import …` line; confirm `__init__`
      still re-exports the identical public surface.
- [ ] 4.2 Grep `src/` and `tests/` for residual `_legacy` references; repoint any
      helper read through a `_legacy.*` alias to its canonical
      `fmo.idempotency` / `fmo.quota_normalize` / `_helpers` source.

## 5. Close out

- [ ] 5.1 `make check` clean (resolve the pyright `Any`-laundering errors that
      surface once the wrappers are gone).
- [ ] 5.2 Run the focused stage test files green, then the **full** pytest suite
      under both entry points as the behavior-preservation oracle.
- [ ] 5.3 Clear the bound scenarios from `tests/spec_coverage_pending.txt` and
      `EXPECTED_ACTIVE_PENDING` once green.
- [ ] 5.4 `git diff --check`; `openspec validate refactor-drain-stages-apply --strict`.
