...
- [ ] 1.1 Add a failing test: a summary stating `N tokens per month` yields a
  claim with `metric="tokens"`, the right amount and window — not a `requests`
  claim and not a `missing_amount` error.
- [ ] 1.2 Add a failing test: a summary stating `N requests per day` still yields
  `metric="requests"` (no regression).
- [ ] 1.3 Add a failing test: a summary stating both a request/day and a
  token/month limit captures both axes.
- [ ] 1.4 Add a failing test: a summary expressing only `N requests per minute`
  (or per hour) does NOT activate a capacity rule — the endpoint stays without a
  confirmed budget rule.
- [ ] 1.5 Add a failing test: an Instructor inspector claim with `metric="tokens"`
...
- [ ] 2.1 Replace the hardcoded `metric="requests"` in `extract_summary_claim`
  with axis detection over the summary text.
- [ ] 2.2 Extend `_extract_amount`/`_extract_window` to recognise token phrasing
  (`tokens per day/month`) alongside request phrasing; capture both axes when both
  are present.
- [ ] 2.3 Route sub-day request windows out: a `requests` claim with window
  `minute`/`hour` SHALL NOT become a capacity rule (reactive, left to OmniRoute).
- [ ] 2.4 Apply the same sub-day-request routing to inspector-returned claims;
  otherwise carry the inspector `metric` through unchanged.
- [ ] 2.5 Keep `summary_confidence_cap`, worsen-quota safe-mode and the
  deterministic validator unchanged.

## 3. Verification

- [ ] 3.1 Run targeted tests: `test_quota.py`, `test_quota_research_ingestion.py`,
  `test_quota_normalize.py`.
- [ ] 3.2 Run `.venv/bin/python -m pytest -q`.
- [ ] 3.3 Run `npx --yes @fission-ai/openspec@latest validate update-quota-research-capture-axis --strict`.
