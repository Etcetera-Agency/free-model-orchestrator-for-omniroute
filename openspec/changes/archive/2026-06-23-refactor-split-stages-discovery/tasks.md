...
- [x] 1.1 Write failing test: the `_production_stage_adapters()` table and
...
- [x] 1.2 Write failing test: discovery, quota, and access stages live in
...
- [x] 2.1 Create `src/fmo/composition_stages/__init__.py` re-exporting the full
...
- [x] 2.2 Keep cross-cluster helpers in place so later-cluster stages still
      import them unchanged this slice.

## 3. Extract the discovery, quota, and access clusters

- [x] 3.1 Move the discovery stage + helpers into `discovery.py`.
- [x] 3.2 Move the quota research/sync stages + helpers into `quota.py`.
- [x] 3.3 Move the access-classification stage + helpers into `access.py`.
- [x] 3.4 Fix the pyright errors in the three moved modules.

## 4. Close out

- [x] 4.1 `make check` clean.
- [x] 4.2 Run `tests/test_discovery.py`, `tests/test_quota.py`,
      `tests/test_composition.py`; full suite intentionally deferred by
      automation instruction.
- [x] 4.3 Remove the two bound scenarios from `tests/spec_coverage_pending.txt`
      and `EXPECTED_ACTIVE_PENDING`.
- [x] 4.4 `openspec validate refactor-split-stages-discovery --strict`.
