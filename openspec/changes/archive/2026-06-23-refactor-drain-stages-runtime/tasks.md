...
- [x] 1.1 Write failing test: `_probing_stage`, `_telemetry_sync_stage`,
...
- [x] 2.1 Move `_probing_stage` into `probing.py`, sourcing `_remaining_requests`
      from `quota_normalize.remaining_amount`; delete the `_legacy` wrapper.
- [x] 2.2 Move `_telemetry_sync_stage` into `telemetry.py`; delete the wrapper.

## 3. Drain inventory

- [x] 3.1 Move `_hermes_inventory_stage`, `_read_hermes_inventory`,
      `_run_hermes_inspector` into `inventory.py`; delete the wrappers.

## 4. Drain roles

- [x] 4.1 Move `_role_lifecycle_stage`, `_role_scoring_stage` and the
...
- [x] 5.1 Repoint moved bodies at canonical helpers instead of `_legacy.*`
...
- [x] 5.3 Run the focused role/telemetry/inventory/probing composition + spec docs
      tests; defer the full suite to the terminal `refactor-drain-stages-apply`
      slice.
- [x] 5.4 `git diff --check`; `openspec validate refactor-drain-stages-runtime --strict`.
