...
- [x] 1.1 Add a failing test: `auto_router_tail` parses ordered entries with `id`
...
- [x] 1.2 Add a failing test: a router endpoint is NOT AA-scored — no
...
- [x] 1.3 Add a failing test: a router with a missing band metric is exempt from
...
- [x] 1.4 Add a failing test: in `build_priority_combo`, scored endpoints keep
  AA-quality-ascending order and qualifying routers are appended after the last
  scored endpoint in `auto_router_tail` order.
- [x] 1.5 Add a failing test: a router whose config-declared `input` is text-only
...
- [x] 1.6 Add a failing test: routers do not consume the scored-slot
  `per_pool_cap`; the tail is bounded by `auto_router_tail` length.
- [x] 1.7 Add a failing test: a role whose only eligible free endpoints are
  routers yields a valid router-only combo in config order (no paid fallback).
- [x] 1.8 Bind scenarios with `@pytest.mark.spec(...)` and stage them in
  `tests/spec_coverage_pending.txt` until their tests land.

## 2. Implementation

- [x] 2.1 Add the structured `auto_router_tail` setting (per-entry `id` +
...
- [x] 2.2 In the scoring path, short-circuit AA scoring for configured routers:
  skip `aa_subscore`/`benchmark_fit` and emit no uncertainty penalty; keep all
  other non-quality eligibility hard filters.
- [x] 2.3 In `quality.py`, exempt configured routers from the band/quality-gate:
  do not mark them `unverifiable` or exclude them on a missing band metric.
- [x] 2.4 Carry router membership from the scoring stage into the score map that
  `build_priority_combo` consumes in `composition_stages.py`.
- [x] 2.5 In `build_priority_combo`, build the scored head (AA ascending) then
...
- [x] 3.1 Run targeted tests: config, scoring, quality, allocation, composition.
- [x] 3.2 Run `.venv/bin/python -m pytest -q`. (Skipped by user request: no full test after each slice.)
- [x] 3.3 Run `npx --yes @fission-ai/openspec@latest validate add-auto-router-fallback-tail --strict`.
