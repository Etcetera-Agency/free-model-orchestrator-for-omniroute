...
- [x] 1. TEST: when a desired `fmo-` diff's combo id is **absent** from the live
...
- [x] 2. TEST: an absent combo is skipped without failing the run — other present
  combos in the same apply still mutate and smoke-test → implement skip-not-fail.
- [x] 3. TEST: apply never issues a DELETE for any combo → assert no delete call
  on any path (success, drift, rollback).
- [x] 4. TEST: an existing `fmo-` combo still rebalances with the full drift /
  smoke / rollback behavior unchanged → regression guard.
- [x] 5. TEST: `ComboApplier.apply()` refuses an id not in its `current` map
  → add the defense-in-depth guard.
- [x] 6. Remove the unknown-role immediate-inventory expectation **at
