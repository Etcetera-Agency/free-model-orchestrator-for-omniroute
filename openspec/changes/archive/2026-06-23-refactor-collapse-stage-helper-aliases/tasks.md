...
- [x] 2.1 Remove the `_canonical_slug = canonical_slug` … `_remaining_amount =
...
- [x] 2.2 Repoint every drained cluster call site to the canonical names:
...
- [x] 2.3 Remove the `from ._helpers import _canonical_slug …` re-export lines for
      the five helpers from `composition_stages/__init__.py`.

## 3. Close out

- [x] 3.1 Grep `src/` and `tests/` for any residual underscore-aliased helper
...
- [x] 3.2 `make check` clean.
- [x] 3.3 Run the full pytest suite under both entry points as the
      behavior-preservation oracle.
- [x] 3.4 `git diff --check`; `openspec validate refactor-collapse-stage-helper-aliases --strict`.
