...
- [x] 1.1 Add a failing test: `refine_global_tokens_per_request` aggregates
...
- [x] 1.2 Add a failing test: the refined factor is clamped to within
  `±max_change_ratio` of `current` for both an extreme downward and an extreme
  upward week. *(landed)*
- [x] 1.3 Add a failing test: `recompute_derived_capacities` recomputes req/day
  for `summary` and `calibrated` endpoints under the new factor and omits `live`
  endpoints entirely. *(landed)*
- [x] 1.4 Add a failing test: `tokens_per_request_recalibration_cron` is validated
  as a 5-field cron and rejected when malformed. *(config validation)*
- [x] 1.5 Add a failing test for the weekly job: on a non-matching timestamp it is
...
- [x] 1.6 Add a failing test: the job acquires its run lock and is a no-op (no
...
- [x] 2.1 `quota_normalize.refine_global_tokens_per_request` + `CalibrationObservation`
  + `recompute_derived_capacities` + `DERIVED_SOURCES`. *(landed)*
- [x] 2.2 `config.tokens_per_request_recalibration_cron` setting + cron validation.
  *(landed)*
- [x] 2.3 Persistence: read accumulated calibration observations
...
- [x] 2.4 Weekly recalibration job: cron-gated tick that, under a run lock and a
...
- [x] 2.5 Wire the job's cron from `tokens_per_request_recalibration_cron` into the
  scheduler tick path alongside the daily run.

## 3. Verification

- [x] 3.1 Run targeted tests: `test_quota_normalize.py`, config, scheduler.
- [x] 3.2 Run `.venv/bin/python -m pytest -q`. (Skipped by user request: no full test after each slice.)
- [x] 3.3 Run `npx --yes @fission-ai/openspec@latest validate add-weekly-tpr-recalibration --strict`.
