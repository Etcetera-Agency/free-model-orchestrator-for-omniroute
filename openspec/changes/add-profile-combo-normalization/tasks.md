...
- [ ] 1. TEST: a raw slot `vision: google/gemini-2.5-flash` is rewritten to the
...
- [ ] 2. TEST: a slot pointing at a non-existent combo id is rewritten the same
...
- [ ] 3. TEST: a slot already on an existing `fmo-` combo, or on `auto`/empty, is
  left unchanged → implement the keep path.
- [ ] 4. TEST: `--dry-run` reports the planned rewrites and writes nothing (no
  file changes, no backups) → implement dry-run.
- [ ] 5. TEST: on apply, each touched `config.yaml` is backed up before an atomic
...
- [ ] 6. TEST: `normalize-profiles` is in the CLI surface and dispatches to the
