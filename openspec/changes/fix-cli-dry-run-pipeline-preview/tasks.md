...
- [ ] 1. TEST: `diff --dry-run` runs the diff stage and returns the stage's real
  outcome (not unconditional success) without mutating OmniRoute → remove the
  blanket dry-run short-circuit in `run_cli`.
- [ ] 2. TEST: `apply --dry-run` runs the precondition/validation path and
...
- [ ] 3. TEST: `full --dry-run` runs all stages read-only and surfaces the real
  worst-status exit code, writing no OmniRoute mutation → verify end to end.
- [ ] 4. TEST: metadata `--dry-run` behavior is unchanged (external fetch
  validated, no DB writes) → regression guard.
- [ ] 5. Bind tests with `@pytest.mark.spec("cli-and-operations::...")`, drop
