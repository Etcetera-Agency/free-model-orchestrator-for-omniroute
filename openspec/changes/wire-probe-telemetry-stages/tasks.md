...
- [ ] 1.1 Add a failing effect test: `probing` runs only for `confirmed`-free
  endpoints with reserved capacity and never exceeds confirmed free capacity.
- [ ] 1.2 Add a failing effect test: `probing` persists probe results and
  excludes endpoints whose probe fails.
- [ ] 1.3 Add a failing effect test: `telemetry-sync` writes normalized telemetry
  rows used by scoring.
- [ ] 1.4 Add a failing effect test: `quota-sync` writes synced remaining-quota
...
- [ ] 2.1 Add a `probing` adapter calling `probes`, gated on confirmed-free +
  reserved capacity, persisting results.
- [ ] 2.2 Add a `telemetry-sync` adapter calling `telemetry`, persisting
  normalized rows.
- [ ] 2.3 Add a `quota-sync` adapter calling `quota_manager` +
...
- [ ] 3.1 Run targeted tests for probes, telemetry, quota, composition.
