...
def test_quality_gate_hard_prefilter_unverifiable_and_index_change():
    assert (
        evaluate_quality_gate(
            {"agentic_index": 30}, metric="agentic_index", value=45, index_version="v1", current_version="v1"
        ).eligible
        is False
    )
    above = evaluate_quality_gate(
    ...
    changed = evaluate_quality_gate(
        {"coding_index": 50}, metric="coding_index", value=10, index_version="v1", current_version="v2"
    )
